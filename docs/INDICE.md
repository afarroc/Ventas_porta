# Índice de Documentación

## Raíz `/`

- `README.md`: inicio rápido, estado actual, estructura del proyecto y mapa de documentación.
- `DEPLOYMENT.md`: configuración de base de datos y arranque del servicio.
- `HANDOFF_2026-06-11.md`: fix de ubigeo y flujo de departamentos/provincias/distritos.

## `docs/`

- `documentacion.md`: documentación principal del sistema.
- `HISTORIAL.md`: registro cronológico de cambios.
- `DEV_REFERENCE.md`: referencia técnica de arquitectura, apps separadas, servicios y signals.
- `queries_referenciadas.md`: queries de trazabilidad Lead → Venta y postventa.
- `DEPLOYMENT.md`: notas de despliegue duplicadas en raíz.
- `HANDOFF_2026-06-11.md`: fix de ubigeo y flujo de departamentos/provincias/distritos.
- `HANDOFF_2026-06-05_ventas.md`: handoff de ventas.
- `HANDOFF_2026-06-06_venta_refactor.md`: refactor del modelo Venta.
- `HANDOFF_2026-06-07_apps_separacion.md`: separación de apps postventa, despacho y courier.
- `HANDOFF_2026-06-07_trazabilidad.md`: trazabilidad Lead → Venta.
- `HANDOFF_2026-06-08_estado_actual.md`: estado de sprints, bugs críticos y reglas retail.
- `HANDOFF_2026-06-10_modelo_venta_refactor.md`: refactor del modelo Venta.

## Orden recomendado de lectura

1. `../README.md`
2. `documentacion.md`
3. `HISTORIAL.md`
4. `DEV_REFERENCE.md`
5. `queries_referenciadas.md`
6. `HANDOFF_2026-06-08_estado_actual.md`
7. Handoffs anteriores según fecha.
